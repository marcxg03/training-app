...
14. **Make `activatePlan` atomic (transactional RPC).** Currently deactivate-all-then-activate-target as two non-transactional writes; a partial failure leaves zero active plans (dead-looking app until re-load). Replace with a Postgres function / transactional RPC so the active-plan flip is atomic (the "exactly one active plan" invariant holds). Roast S4 F2. 2026-09-23.
